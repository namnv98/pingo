 ...
 * sách hội thoại của bạn" — xem {@link ConversationMembershipRegistry}. Chỉ vài route đơn giản nên vẫn
...
public class RestApiVerticle extends AbstractVerticle {
  ...
  private final Map<RouteKey, Consumer<HttpServerRequest>> routes =
      Map.of(
          new RouteKey(HttpMethod.GET, "/healthcheck"), r -> handleHealthcheck(r.response()),
          new RouteKey(HttpMethod.GET, "/messages"), this::handleListMessages,
          new RouteKey(HttpMethod.GET, "/users"), this::handleListUsers,
          new RouteKey(HttpMethod.PUT, "/users"), this::handleSetUsername,
          new RouteKey(HttpMethod.GET, "/conversations"), this::handleListConversations,
          new RouteKey(HttpMethod.POST, "/register"), this::handleRegister,
          new RouteKey(HttpMethod.POST, "/login"), this::handleLogin);
  ...
  private void handleRequest(HttpServerRequest request) {
    ...
    var handler = routes.get(new RouteKey(request.method(), request.path()));
    if (handler == null) {
